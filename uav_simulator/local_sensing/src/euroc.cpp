...
typedef message_filters::sync_policies::ApproximateTime<sensor_msgs::msg::Image, geometry_msgs::TransformStamped>
    approx_policy;
...
rclcpp::Publisher<sensor_msgs::msg::Image>>::SharedPtr pub_posedimage;
...
void image_pose_callback(
    const sensor_msgs::msg::ImageConstPtr &image_input,
...
int main(int argc, char **argv) {
  ...
  nh->declare_parameter<int>("cam_width", 0);
  nh->declare_parameter<int>("cam_height", 0);
  nh->declare_parameter<double>("cam_fx", 0);
  nh->declare_parameter<double>("cam_fy", 0);
  nh->declare_parameter<double>("cam_cx", 0);
  nh->declare_parameter<double>("cam_cy", 0);
  ...
  if (nh.hasParam("cam_k1") &&
      ...
      nh.hasParam("cam_r2")) {
    float k1, k2, r1, r2;
    nh->declare_parameter<float>("cam_k1", 0);
    nh->declare_parameter<float>("cam_k2", 0);
    nh->declare_parameter<float>("cam_r1", 0);
    nh->declare_parameter<float>("cam_r2", 0);

    nh->get_parameter("cam_k1", k1);
    nh->get_parameter("cam_k2", k2);
    nh->get_parameter("cam_r1", r1);
    nh->get_parameter("cam_r2", r2);
  ...
  string cloud_path;
  nh->declare_parameter<std::string>("cloud_path", string(""));
  nh->get_parameter("cloud_path", cloud_path);
  printf("cloud file %s\n", cloud_path.c_str());
